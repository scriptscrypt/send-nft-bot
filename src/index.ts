...
bot.action(/genstandard:(.+)/, async (ctx) => {
  try {
    ...
    const storedImage = await storeImage(image_bytes, filename, userId, prompt) as ImageData;
...
bot.action('wallet:view', async (ctx) => {
  try {
    ...
    if (!wallet) {
      return ctx.reply('You don\'t have a wallet yet. Create one first!',
        Markup.inlineKeyboard([
          [Markup.button.callback('Create Wallet', 'wallet:create')],
          [Markup.button.callback('« Back to Wallet Menu', 'open_wallet')]
        ])
...
bot.action('wallet:delegate', async (ctx) => {
  try {
    ...
    if (success) {
      await ctx.reply('✅ Server session enabled successfully! You can now use advanced features.',
        Markup.inlineKeyboard([
          [Markup.button.callback('« Back to Wallet Menu', 'open_wallet')]
        ])
      );
...
bot.action('wallet:revoke', async (ctx) => {
  try {
    ...
    if (success) {
      await ctx.reply('✅ Server session revoked successfully!',
        Markup.inlineKeyboard([
          [Markup.button.callback('« Back to Wallet Menu', 'open_wallet')]
        ])
      );
...
bot.action(/pump:(.+)/, async (ctx) => {
  try {
    ...
    ctx.reply('Pump launch feature coming soon!',
      Markup.inlineKeyboard([
        [Markup.button.callback('« Back to Menu', 'back_to_menu')]
      ])
    );
...
bot.action(/mint:(.+)/, async (ctx) => {
  try {
    ...
    ctx.reply('NFT minting feature coming soon!',
      Markup.inlineKeyboard([
        [Markup.button.callback('« Back to Menu', 'back_to_menu')]
      ])
    );
